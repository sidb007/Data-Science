...
o=open("C:\Users\sidb\Documents\Python Important Scripts\Working Dir\hw9\split1.train")
...
for i in con_list.keys():
    p_wkc=0.0
    nkc=con_list[i]
    p_wkc=(nkc+1)/(len_c+len_vocab)    
    con_list[i]=math.log(p_wkc)
    
for i in lib_list.keys():
    p_wkl=0.0
    nkl=lib_list[i]
    p_wkl=(nkl+1)/(len_l+len_vocab)    
    lib_list[i]=math.log(p_wkl)
    
o1=open("C:\Users\sidb\Documents\Python Important Scripts\Working Dir\hw9\split1.test")
